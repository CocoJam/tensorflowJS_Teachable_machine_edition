...
function integrateWasmJS() {
  ...
  var wasmTextFile = 'dlib.wast';
  var wasmBinaryFile = 'dlib.wasm';
  var asmjsCodeFile = 'dlib.temp.asm.js';
...
/* global initializers */  __ATINIT__.push({ func: function() { __GLOBAL__sub_I_array_cpp() } });
...
// {{PRE_LIBRARY}}
  ...
  function __emval_as() {
  err('missing function: _emval_as'); abort(-1);
  }

  function __emval_decref() {
  err('missing function: _emval_decref'); abort(-1);
  }

  function __emval_get_global() {
  err('missing function: _emval_get_global'); abort(-1);
  }

  function __emval_get_module_property() {
  err('missing function: _emval_get_module_property'); abort(-1);
  }

  function __emval_get_property() {
  err('missing function: _emval_get_property'); abort(-1);
  }

  function __emval_incref() {
  err('missing function: _emval_incref'); abort(-1);
  }

  function __emval_new() {
  err('missing function: _emval_new'); abort(-1);
  }

  function __emval_new_array() {
  err('missing function: _emval_new_array'); abort(-1);
  }

  function __emval_new_cstring() {
  err('missing function: _emval_new_cstring'); abort(-1);
  }

  function __emval_run_destructors() {
  err('missing function: _emval_run_destructors'); abort(-1);
  }

  function __emval_take_value() {
  err('missing function: _emval_take_value'); abort(-1);
  }

  function __emval_typeof() {
  err('missing function: _emval_typeof'); abort(-1);
  }
...
Module["asm"] = asm;
var __GLOBAL__sub_I_array_cpp = Module["__GLOBAL__sub_I_array_cpp"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return Module["asm"]["__GLOBAL__sub_I_array_cpp"].apply(null, arguments) };
